# SPDX-License-Identifier: Apache-2.0
# Ruff rule PERF203 (Perflint): try except in loop
# Clean-room Rego implementation for the Vulnetix CLI input model.

package vulnetix.rules.ruff_perf203

import rego.v1

metadata := {
	"id": "RUFF-PERF203",
	"name": "try except in loop",
	"description": "`try`-`except` within a loop incurs performance overhead",
	"help_uri": "https://docs.astral.sh/ruff/rules/try-except-in-loop/",
	"languages": ["python"],
	"severity": "low",
	"level": "warning",
	"kind": "sast",
	"cwe": [],
	"capec": [],
	"attack_technique": [],
	"cvssv4": "",
	"cwss": "",
	"tags": ["python", "ruff", "perflint", "perf"],
	"ruff_code": "PERF203",
	"ruff_linter": "Perflint",
	"ruff_name": "try-except-in-loop",
	"ruff_since": "v0.0.276",
	"ruff_fix": "None",
}

_is_py(path) if endswith(path, ".py")
_is_py(path) if endswith(path, ".pyw")

findings contains finding if {
	some path in object.keys(input.file_contents)
	_is_py(path)
	lines := split(input.file_contents[path], "\n")
	some i, line in lines
	regex.match(`try:.*\n.*\w+\.append\s*\(`, line)
	finding := {
		"rule_id": metadata.id,
		"message": "try-except within a loop incurs performance overhead",
		"artifact_uri": path,
		"severity": metadata.severity,
		"level": metadata.level,
		"start_line": i + 1,
		"snippet": line,
	}
}
