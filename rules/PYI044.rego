# SPDX-License-Identifier: Apache-2.0
# Ruff rule PYI044 (flake8-pyi): future annotations in stub
# Clean-room Rego implementation for the Vulnetix CLI input model.

package vulnetix.rules.ruff_pyi044

import rego.v1

metadata := {
	"id": "RUFF-PYI044",
	"name": "future annotations in stub",
	"description": "`from __future__ import annotations` has no effect in stub files, since type checkers automatically treat stubs as having those semantics",
	"help_uri": "https://docs.astral.sh/ruff/rules/future-annotations-in-stub/",
	"languages": ["python"],
	"severity": "low",
	"level": "warning",
	"kind": "sast",
	"cwe": [],
	"capec": [],
	"attack_technique": [],
	"cvssv4": "",
	"cwss": "",
	"tags": ["python", "ruff", "flake8-pyi", "pyi"],
	"ruff_code": "PYI044",
	"ruff_linter": "flake8-pyi",
	"ruff_name": "future-annotations-in-stub",
	"ruff_since": "v0.0.273",
	"ruff_fix": "Sometimes",
}

_is_py(path) if endswith(path, ".py")
_is_py(path) if endswith(path, ".pyw")

# NOTE: This rule requires AST-level analysis and cannot be fully implemented
# via text patterns. This is a stub that flags files for manual review.
# Full implementation would require a Python AST parser.

# Stub: no findings by default (requires AST analysis)
findings := set()
