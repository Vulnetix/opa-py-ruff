# SPDX-License-Identifier: Apache-2.0
# Ruff rule ISC002 (flake8-implicit-str-concat): multi line implicit string concatenation
# Clean-room Rego implementation for the Vulnetix CLI input model.

package vulnetix.rules.ruff_isc002

import rego.v1

metadata := {
	"id": "RUFF-ISC002",
	"name": "multi line implicit string concatenation",
	"description": "Implicitly concatenated string literals over multiple lines",
	"help_uri": "https://docs.astral.sh/ruff/rules/multi-line-implicit-string-concatenation/",
	"languages": ["python"],
	"severity": "low",
	"level": "warning",
	"kind": "sast",
	"cwe": [],
	"capec": [],
	"attack_technique": [],
	"cvssv4": "",
	"cwss": "",
	"tags": ["python", "ruff", "flake8-implicit-str-concat", "isc"],
	"ruff_code": "ISC002",
	"ruff_linter": "flake8-implicit-str-concat",
	"ruff_name": "multi-line-implicit-string-concatenation",
	"ruff_since": "v0.0.201",
	"ruff_fix": "None",
}

_is_py(path) if endswith(path, ".py")
_is_py(path) if endswith(path, ".pyw")

findings contains finding if {
	some path in object.keys(input.file_contents)
	_is_py(path)
	lines := split(input.file_contents[path], "\n")
	some i, line in lines
	regex.match(`["\'].*\\\s*\n\s*["\']`, line)
	finding := {
		"rule_id": metadata.id,
		"message": "Implicitly concatenated string literals over multiple lines",
		"artifact_uri": path,
		"severity": metadata.severity,
		"level": metadata.level,
		"start_line": i + 1,
		"snippet": line,
	}
}
